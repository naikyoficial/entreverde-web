/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════
   Structure:
   Block 1 — Core (works without GSAP):
     lazy images · video · grain · hero animations · loader
     stats · mobile menu · FAQ · CTA buttons · smooth scroll
     form submit · nav scroll (native)
   Block 2 — GSAP enhancements (optional):
     magnetic buttons · card spotlight · parallax
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function () {
    ...
    if (src) src.addEventListener('error', showFallback, { once: true });

    function tryPlay() {
      const p = video.play();
      if (p && p.catch) p.catch(err => {
        if (!err || err.name !== 'AbortError') showFallback();
      });
    }
  ...
  function runHeroAnimations() {
    ...
    try {
      ...
      tl.from('.h1-line:first-child .h1-inner', { yPercent: 110, duration: 1.15 }, 0.1)
        ...
        .to('.hero-ship',     { y: 0, opacity: 1, duration: 0.7 }, 0.75)
  ...
  /* ── Nav scroll — native listener (works without GSAP) ── */
  ...
  /* ─────────────────────────────────────
     BLOCK 2 · GSAP ENHANCEMENTS
     Everything below requires GSAP.
     If CDN fails, the site still works.
  ───────────────────────────────────── */
  ...
  /* Override nav with ScrollTrigger for precise scroll detection */
  ...
  if (!isTouch && !reduceMotion) {
    ...
    gsap.from('.sustrato-content .section-label, .sustrato-heading, .sustrato-text, .s-benefit', {
      x: 60, opacity: 0, duration: 1, stagger: 0.1, ease: 'power3.out',
    ...
    gsap.from('.cta-card', {
      y: 60, opacity: 0, scale: 0.95, duration: 1.1, ease: 'back.out(1.5)',
      scrollTrigger: { trigger: '.cta-card', start: 'top 82%' }
    });
    gsap.from('.product-visual', {
      x: 50, opacity: 0, duration: 1.1, delay: 0.2, ease: 'power3.out',
    ...
    document.querySelectorAll('[data-tilt]').forEach(card => {
      card.addEventListener('mousemove', e => {
        ...
        gsap.to(card, { rotateY: dx * 10, rotateX: -dy * 10, transformPerspective: 800, duration: 0.4, ease: 'power2.out' });
      });
    ...
    progressBar.style.cssText = 'position:fixed;top:0;left:0;height:2px;width:0;background:var(--green);z-index:9990;pointer-events:none;transition:width 0.1s linear';
